...
import { addresDetail, addressUpdate } from "../../lib/api/AddressApi";
...
async function fetchAddress() {
	const response = await addresDetail(token.value, id, addressId);
	const result = await response.json();

	if (response.status === 200) {
		address.city = result.data.city;
		address.country = result.data.country;
		address.postal_code = result.data.postal_code;
		address.province = result.data.province;
...
async function handleSubmit() {
	const response = await addressUpdate(token.value, id, {
		...address,
		addressId,
	});
	const result = await response.json();
	console.log(result);
	if (response.status === 200) {
		await alertSuccess("Successfully update address");
	} else {
		await alertError(result.errors);
	}
}

onMounted(async () => await Promise.all([fetchAddress(), fetchContact()]));
</script>
		...
		<div class="p-8">
			...
			<form v-on:submit.prevent="handleSubmit">
				...
				<div class="flex justify-end space-x-4">
					<RouterLink
						:to="`/dashboard/contacts/${id}/detail`"
